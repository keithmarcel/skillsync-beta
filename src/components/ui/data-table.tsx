...
export default function DataTable({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Table Container */}
      <div className="w-full max-w-[1232px] rounded-xl border border-[#E5E5E5] bg-[#FCFCFC] p-2">
        {/* Table Header */}
        <div className="flex items-center self-stretch px-6 h-[50px] bg-[#F9FAFB] text-[#114B5F] text-xs font-semibold uppercase tracking-wide rounded-t-xl border-b border-[#E5E7EB]">
          {columns.map((column, index) => (
            <div 
              key={column.key}
              className={`flex-1 ${index === columns.length - 1 ? 'text-right' : ''}`}
            >
              {index === columns.length - 1 ? 'Actions' : column.label}
            </div>
          ))}
        </div>
                ...
                >
                  {colIndex === 0 ? (
                    ...
                    <Link 
                      ...
                      className="text-inherit group-hover:text-[#0694A2] transition-colors font-normal"
                    >
